...
private router : Router
 ...
 .subscribe((transformedPostData)=>{
  ...
  addPost(title : string, content : string, image : File){
    ...
    const post : Post = { id : null,title : title, content : content, imagePath : null, creater : null};
 ...
 updatePost(id : string, title : string, content : string, image : File | string){
  ...
  .subscribe(response =>{
    ...
    const  oldPostsIndex = updatedPosts.findIndex(p =>{
      p.id === id
    })
  //  const  post: Post = {
  //    id : id,
  ...
  return this.http.get<{_id : string, title : string, content : string, imagePath : string; creater : string}>("http://localhost:3000/api/posts/" +id)
